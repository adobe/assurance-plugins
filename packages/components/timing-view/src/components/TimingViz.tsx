...
import type { BackgroundColorValue, Responsive } from "@react-types/shared";
        ...
        {chain.map((event: Event, index) => {
          const width =
            index === 0
              ? 6
              : Math.max(
                  (event.timestamp - chain[index - 1].timestamp) * scale,
                  6,
                );
          return (
            <EventTooltip
            ...
            >
              <View
                backgroundColor={
                  COLORS[index] as Responsive<BackgroundColorValue>
                }
                height={30}
                minWidth={width}
                width={width}
...
const TimingViz = ({ branch, scale }) => {
  ...
  const flatten = (branch: Branch, chain?: Chain) => {
    ...
    if (Object.keys(branch.children).length) {
      Object.values(branch.children).forEach((child) =>
        flatten(child, newChain),
      );
    } else {
  ...
  return (
    <Flex direction="column" gap="size-100" marginY="size-200">
      {chains.map((chain) => (
        <TimingVizRow chain={chain} size={300} scale={scale} />
      ))}
    </Flex>
  );
};
