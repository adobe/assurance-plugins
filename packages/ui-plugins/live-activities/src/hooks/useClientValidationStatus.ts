...
function useClientValidationStatus() {
    ...
    if (!ecid) { return 'no-ecid'; }
    if (!messagingVersion) { return `messaging-not-installed-${clientType}`; }
    if (!pushToken) { return `no-token-${clientType}`; }
    if (!dataStream) { return 'edge-not-configured'; }
    if (!messagingEventDataset) { return 'messaging-not-configured'; }
    return false;
}
