...
const BAR_HEIGHT = 56;
const getSelectedIndex = (selected, events) => R.findIndex(check => check && check.uuid === (selected || {}).uuid, events || []) || 0;
const Timeline = () => {
    ...
    const isSelected = e => e && e.uuid && R.findIndex(row => row && row.uuid === e.uuid, selected || []) >= 0;
    ...
    const scrubberLines = useMemo(() => events.map((e, index) => isVisible(index) && (React.createElement(ScrubberLine, { key: e.uuid, index: index, highlightColor: e.highlight, event: e, onPress: () => {
            selectEvents([e.uuid]);
        }, isSelected: isSelected(e), eventSize: eventSize }))), [events, barPosition, width, selected]);
    return (React.createElement(Flex, { gap: "size-50", marginEnd: 10 },
        React.createElement("div", { ref: outer, style: {
                height: BAR_HEIGHT,
                pointerEvents: 'none',
                width: 30,
                flexGrow: 5,
            ...
            React.createElement("div", { style: {
                    ...
                    overflowY: 'hidden',
                    pointerEvents: 'auto'
                }, ref: bar }, events?.length && (React.createElement(View, { width: events.length * eventSize }, scrubberLines)))),
        selected?.length ? (React.createElement(View, { marginTop: 8 },
            React.createElement(ActionButton, { isQuiet: true, onPress: () => {
                    selectEvents([]);
                } },
                React.createElement(CloseCircle, { size: 'S' })))) : null));
};
